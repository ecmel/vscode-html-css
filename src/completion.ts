...
export class SelectorCompletionItemProvider
  implements CompletionItemProvider, Disposable
{
  ...
  readonly findExtended =
    /(?:{{<|{{>|{%\s*extends|@extends\s*\()\s*("|')?([./A-Za-z_0-9\\\-]+)\1\s*(?:\)|%}|}})/i;
  ...
  getPath(uri: Uri, path: string, ext?: string): string {
    ...
    return folder
      ? join(isAbsolute(path) ? folder.uri.fsPath : dirname(uri.fsPath), name)
  ...
  async validate(document: TextDocument): Promise<Diagnostic[]> {
    ...
    while ((attribute = findAttribute.exec(text)) !== null) {
      ...
      while ((value = findSelector.exec(attribute[3])) !== null) {
        if (
          !(attribute[1] === "id" ? context.ids : context.classes).has(value[1])
        ) {
